...
INPUT_ROOT = r"txt"
OUTPUT_ROOT = r"txt_improve"


CATEGORIES = {"ADI", "DEB", "LYM", "MUC", "MUS", "NOR", "STR", "TUM"}

# Prompt
PROMPT_FILE = r"Deepseek-refine.txt"
...
def load_category_prompts(path: str) -> Dict[str, str]:
    ...
    if not blocks:
        raise ValueError(f"No valid category prompts found in {path}. Make sure headers like 'ADI:' or 'ADI：' exist.")
    return blocks
...
def build_prompt_for_category(raw_text: str, category: str, prompt_map: Dict[str, str]) -> str:
...
def improve_text(client: OpenAI, final_prompt: str, max_retries: int = MAX_RETRIES) -> Tuple[str, Optional[str]]:
    last_err = None
...
def process_file(client: OpenAI,
    ...
    try:
        original = read_text_with_fallback(file_path).strip()
    except Exception as e:
        print(f"⚠️ Read failed (skip): {file_path} -> {e}")
        return None
    ...
    out_opt = os.path.join(out_dir, f"{base}_opt.txt")
    out_reason = os.path.join(out_dir, f"{base}_reason.txt")


    if os.path.exists(out_opt) and (not SAVE_REASONING or os.path.exists(out_reason)):
        return file_path

    try:
        final_prompt = build_prompt_for_category(original, category, prompt_map)
        optimized, reasoning = improve_text(client, final_prompt, MAX_RETRIES)

        with open(out_opt, "w", encoding="utf-8") as f_opt:
            f_opt.write(optimized)
        if SAVE_REASONING and reasoning is not None:
            with open(out_reason, "w", encoding="utf-8") as f_r:
                f_r.write(reasoning)
        return file_path
    except Exception as e:
        print(f"⛔ Failed: {file_path} [{category}] -> {e}")
        return None
